...
const getTransaction = async (req, res, next) => {
    ...
    try {
        const transaction = await transactions.getTransaction(pk_transaction)
...
const createTransaction = async (req, res, next) => {
    ...
    try {
        const transaction = await transactions.createTransaction(pk_transaction, fk_user, description, amount)
        res.status(200).send(transaction)
        next()
